          ...
          <a href={`/product/${product.slug}`} className="block">
            <div className="overflow-hidden rounded-[24px] bg-transparent transition duration-300 ease-out">
              <img
                src={product.image}
                alt={product.name}
                className={product.slug === "quarter-zip" ? "h-[220px] w-full object-contain scale-130 transition duration-500 ease-out group-hover:scale-[1.32] sm:h-[340px]" : "h-[220px] w-full object-contain transition duration-500 ease-out group-hover:scale-[1.03] sm:h-[340px]"}
              />
            </div>
          </a>
    ...
    <main className="min-h-screen bg-white text-[#2F3A4A]">
      <section className="px-4 pb-6 pt-8 text-center sm:px-6 sm:pb-8 sm:pt-10">
        ...
        <div className="mt-6 sm:mt-8">
          <div className="no-scrollbar flex gap-3 overflow-x-auto px-4 pb-2 sm:flex-wrap sm:justify-center sm:px-0 sm:pb-0">
            {categories.map((item) => (
              <a key={item.label} href={item.href} className="flex-shrink-0 whitespace-nowrap rounded-full border border-[#D8D8D4] px-5 py-2 text-sm transition hover:border-[#6F879E] hover:text-[#6F879E]">
                {item.label}
              </a>
            ))}
          </div>
